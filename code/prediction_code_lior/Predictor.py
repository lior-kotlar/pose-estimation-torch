...
# how much better (metres) a candidate must fit the other models with its two wings exchanged before
# harmonize_wing_labels exchanges them. Real label swaps win by ~5 mm; frames where the wings sit
# close together and the call is a coin toss win by under 1 mm, and are left alone. The alignment
# itself lives in wing_labels.py, which needs numpy alone, so the tools that re-run or only ask
# about a movie's ensemble do not have to import a pose estimator
WING_LABEL_SWAP_MIN_MARGIN = wing_labels.WING_LABEL_SWAP_MIN_MARGIN
...
class Predictor2D:
    def __init__(self,
        ...
        self.image_size = self.sparse_box.shape[-2]
    ...
    def save_base_box(self):
        ...
        self.sparse_box.save_to_scipy_sparse_format(box_save_name)
    ...
    def load_preprocessed_box(self):
        ...
        # load box
        box_save_name = os.path.join(self.saved_box_dir, "box.h5")
        self.sparse_box = SparseBox(load_from_sparse=True, sparse_path=box_save_name)

        # load neto wings
        wings_save_name = os.path.join(self.saved_box_dir, "neto_wings.h5")
        self.neto_wings_sparse = SparseBox(load_from_sparse=True, sparse_path=wings_save_name)

        # load body masks
        body_save_name = os.path.join(self.saved_box_dir, "body_masks.h5")
        self.body_masks_sparse = SparseBox(load_from_sparse=True, sparse_path=body_save_name)
    ...
    def get_reprojection_masks(self, points_3D, extend_mask_radius=7, deside_between_new_and_orig=False):
        ...
        for frame in range(self.num_frames):
            for cam in range(self.num_cams):
                fly = self.sparse_box.get_frame_camera_channel_dense(frame, cam, channel_idx=1)
                for wing in range(2):
    ...
    def flip_camera(self, camera_to_flip, frame):
        ...
        left_mask = self.sparse_box.get_frame_camera_channel_dense(frame, camera_to_flip, self.left_mask_ind)
        right_mask = self.sparse_box.get_frame_camera_channel_dense(frame, camera_to_flip, self.right_mask_ind)
        self.sparse_box.set_frame_camera_channel_dense(frame, camera_to_flip, self.left_mask_ind, right_mask)
        self.sparse_box.set_frame_camera_channel_dense(frame, camera_to_flip, self.right_mask_ind, left_mask)
    ...
    def align_time_channels(self):
        ...
        for frame in range(self.num_frames):
            for cam in range(self.num_cams):
                ...
                present = self.sparse_box.get_frame_camera_channel_dense(frame, cam, channel_idx=1)
                cm_present = self.get_fly_cm(present)
                for i, time_channel in enumerate([0, 2]):
                    # fly = self.box[frame, cam, :, :, time_channel]
                    fly = self.sparse_box.get_frame_camera_channel_dense(frame, cam, channel_idx=time_channel)
        ...
        for frame in range(self.num_frames):
            for cam in range(self.num_cams):
                for i, time_channel in enumerate([0, 2]):
                    ...
                    self.sparse_box.set_frame_camera_channel_dense(frame, cam, time_channel, shifted_fly)
    ...
    def predict_per_cam_all_points(self):
        ...
        for cam in range(self.num_cams):
            input = self.sparse_box.get_camera_dense(camera_idx=cam,
                                                                  channels=[0, 1, 2, 3, 4],
                                                                  frames=frames)
        ...
        for cam in range(self.num_cams):
            ...
            for frame in range(self.num_frames):
                ...
                for wing in range(min(num_wings_found, 2)):
                    ...
                    masks_2[:, :, wing] = mask
                self.sparse_box.set_camera_dense(camera_idx=cam, frames=[frame],
    ...
    def set_body_masks(self, opening_rad=6):
        ...
        """
        self.body_masks_sparse = SparseBox(box_path=None,
        ...
        for frame in range(self.num_frames):
            for cam in range(self.num_cams):
                ...
                fly_3_ch[..., 0] = self.sparse_box.get_frame_camera_channel_dense(frame, cam, 0)
                fly_3_ch[..., 1] = self.sparse_box.get_frame_camera_channel_dense(frame, cam, 1)
                fly_3_ch[..., 2] = self.sparse_box.get_frame_camera_channel_dense(frame, cam, 2)
    ...
    def get_neto_wings_masks(self):
        ...
        self.neto_wings_sparse = SparseBox(box_path=None,
                ...
                for wing_num in range(2):
                    other_wing_mask = self.sparse_box.get_frame_camera_channel_dense(frame, cam,
                                                                                     self.num_time_channels + (
                                                                                         not wing_num))
                    wing_mask = self.sparse_box.get_frame_camera_channel_dense(frame, cam,
    ...
    def harmonize_wing_labels(all_points_list, min_margin=WING_LABEL_SWAP_MIN_MARGIN, n_iter=3):
        """Align every candidate's wing labels; see wing_labels.harmonize_wing_labels.

        Kept here so the call sites that reach it through Predictor2D keep working."""
        return wing_labels.harmonize_wing_labels(all_points_list, min_margin=min_margin,
                                                 n_iter=n_iter)

    @staticmethod
    def find_3D_points_optimize_neighbors(all_points_list, max_models=None):
        all_points_list, num_swapped = Predictor2D.harmonize_wing_labels(all_points_list)
        if num_swapped:
            print(f"aligned the wing labels of {num_swapped} (frame, candidate) pairs with the other models", flush=True)
