...
if dltPass
  ...
  [worldXYZout, camPosout, camRout, intrinsicsout, distortionout] = ...
    CameraCalibration.easyWand5.easySBA(imageUV, worldXYZ, camPosout, camRout, intrinsics, intrinsicsFlag, ...
    distortion, distortionFlag);
end
...
if isempty(sCams)
  ...
  if pass2Mode==1 % DLT 2nd pass
    ...
    for i=1:numel(sCams)
      ...
      [R2(:,:,i),t2(:,:,i)]=CameraCalibration.easyWand5.private.EPnP(worldXYZout(edx,:)',xyN(edx,:)');
    end
    ...
    edx=intersect(find(isnan(xyN(:,1))==false),find(isnan(worldXYZout(:,1))==false));
    [R2b,t2b]=CameraCalibration.easyWand5.private.EPnP(worldXYZout(edx,:)',xyN(edx,:)');
