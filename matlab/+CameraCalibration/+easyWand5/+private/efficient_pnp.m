...
%points centroid)
Cw=CameraCalibration.easyWand5.private.define_control_points();

%compute alphas (linear combination of the control points to represent the 3d
%points)
Alph=CameraCalibration.easyWand5.private.compute_alphas(Xw,Cw);

%Compute M
M=CameraCalibration.easyWand5.private.compute_M_ver2(U,Alph,A);

%Compute kernel M
Km=CameraCalibration.easyWand5.private.kernel_noise(M,4); %in matlab we have directly the funcion km=null(M);
...
dim_kerM=1;
X1=Km(:,end);
[Cc,Xc]=CameraCalibration.easyWand5.private.compute_norm_sign_scaling_factor(X1,Cw,Alph,Xw);
...
D=CameraCalibration.easyWand5.private.compute_constraint_distance_2param_6eq_3unk(Km1,Km2);
dsq=CameraCalibration.easyWand5.private.define_distances_btw_control_points();
betas_=inv(D'*D)*D'*dsq;
beta1=sqrt(abs(betas_(1)));
beta2=sqrt(abs(betas_(3)))*sign(betas_(2))*sign(betas_(1));
X2=beta1*Km1+beta2*Km2;

[Cc,Xc]=CameraCalibration.easyWand5.private.compute_norm_sign_scaling_factor(X2,Cw,Alph,Xw);
