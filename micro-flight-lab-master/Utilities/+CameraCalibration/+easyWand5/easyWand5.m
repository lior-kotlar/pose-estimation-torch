...
fprintf('\n')
disp('Welcome to easyWand version 5.5 (October 10, 2017)')
...
if numel(uda.axisPts)==0
  ...
  if size(uda.axisPts,1)<5 & sum(isfinite(axisXYZ(:,1)))~=size(uda.axisPts,1)
    ...
    if nAxPts==2
      ...
      % frame5=frame5-repmat([-0.1061   -2.2462   -0.1443],size(frame5,1),1);
      % frame5=angleaxisRotation(frame5,repmat([0,0,1],size(frame5,1),1),-1.4710);
...
[worldXYZout, camPosout, camRout, intrinsicsout, distortionout] = ...
  CameraCalibration.easyWand5.easySBA(imageUV, worldXYZ, camPos, camR, intrinsics, intrinsicsFlag, ...
...
if c > 10000
  ...
  elseif round(det(R))==-1 && round(det(R2))==1
    ...
    [X1,X1r]=triangulate_v3(R,tv',ptNorm(:,1:4));
    [X2,X2r]=triangulate_v3(R2,tv',ptNorm(:,1:4));
...
if isempty(sCams)
  ...
  if pass2Mode==1 % DLT 2nd pass
    ...
    [worldXYZout2, camPosout2, camRout2, intrinsicsout2, distortionout2] = ...
        CameraCalibration.easyWand5.easySBA(imageUV2, worldXYZ2, camPos2, camR2, intrinsics2, intrinsicsFlag, ...
