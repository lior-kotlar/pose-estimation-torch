...
typedef struct tagSETUP
{
    ...
    uint8_t SamplesPerImage;// Number of samples acquired per image, both 
                            // binary and analog;
    //char BinName[8][11];	(changed to be compatible with MATLAB calllib)
    ...
    //char AnaUnit[8][6];	(changed to be compatible with MATLAB calllib)
	char AnaUnit[8*6];		// Measurement unit for analog channels: max 5
                            // chars/name ended each by a byte = 0
    //char AnaName[8][11];	(changed to be compatible with MATLAB calllib)
    ...
    //char szCinePath[4][OLDMAXFILENAME]; (changed to be compatible with MATLAB calllib)
...
#pragma pack(push,4) //changed for MATLAB use
