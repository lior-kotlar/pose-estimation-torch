...
//MATLAB change, use a PVOID instead of DECLARE_HANDLE
...
typedef struct tagIMFILTER
    {
    int dim;        //square kernel dimension 3,5
    int shifts;     //right shifts of Coef (8 shifts means divide by 256)
    int bias;       //bias to add at end
    int Coef[5*5];  //maximum alocation for a 5x5 filter
    }
IMFILTER, *PIMFILTER;
...
EXIMPROC HRESULT PhGetUseCase(CINEHANDLE hC, int* pCineUseCaseID);
/*****************************************************************************/
