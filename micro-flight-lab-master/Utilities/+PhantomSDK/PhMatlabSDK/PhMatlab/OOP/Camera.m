classdef Camera < handle & ISource
    ...
    methods
        ...
        function cs = GetCinePartitionStatus(this, partNo)
            if (partNo >= PhConConst.CINE_PREVIEW && partNo < PhConConst.FIRST_FLASH_CINE)
        ...
        function lastRamCineNo = GetLastRamCineNo(this)
            lastRamCineNo = (CINE_FIRST + this.GetRamCineCount() - 1);
        end
        
        function lastFlashCineNo = GetLastFlashCineNo(this)
            lastFlashCineNo = (PhConConst.FIRST_FLASH_CINE + this.GetFlashCineCount() - 1);
        end
