...
tic
%home
% addpath('C:\Users\Noamler\Documents\git reps\copy\micro_flight_lab\Insect analysis');
...
cents_in=[];
cluster_params=HullReconstruction.Classes.cluster_and_wings_params_class(how_much_tail,...
                wing_flatness_thresh,num_of_clusters,cents_in);
% initialize variables
mosquito_all_frames(n_frames,1)=UtilitiesMosquito.Classes.mosquito_frame_class; % holds all data for each frame (clusters,angles,...)
...
for fr=start_fr:(start_fr+n_frames-1)
    ...
    for cam_ind=1:3
        allCams.cams_array(cam_ind).load_image(HullReconstruction.Classes.image_insect_class(sparse_movies{cam_ind}{fr}));
    end
    
    q={'image','image','image'};
    
    [ hull_inds,~] = allCams.hull_reconstruction_on_grid(q,voxelSize,volLength,offset_index_size);
    ...
    allCams.currHull3d=HullReconstruction.Classes.hull3d_mosquito_class(hull);
    allCams.currHull3d.clusters_simple(cluster_params);
    allCams.CastClusters;
    UtilitiesMosquito.Functions.BinarizeExtraBodyImage4all(allCams);
    
    [ hull_inds,~] = allCams.hull_reconstruction_on_grid('image',voxelSize,volLength,offset_index_size);
    % translate to easywand space
    hull=[allCams.hull_params.real_coord(1,hull_inds(:,1))',allCams.hull_params.real_coord(2,hull_inds(:,2))',...
        allCams.hull_params.real_coord(3,hull_inds(:,3))'];
    
    allCams.currHull3d.clusters_and_wings(cluster_params);
    
    %% cluster and clean original hull
    % cluster original hull
    mosquito_all_frames(c).all_clusters= UtilitiesMosquito.Functions.clusters_simple(hull,cluster_params);
