...
%     for cam_ind=1:3
    ...
    for cam_ind=cam_inds_rec
        ...
        if cam_ind==1 % cam1 will use full wing boundary
            ...
            wing1_cut_fix=imdilate(threeDtoImage(allCams.all_DLT_coefs(:,cam_ind),cut_wing1_points)...
                ,strel('disk',4))&wing1_bound;
            ...
            wing2_cut_fix=imdilate(threeDtoImage(allCams.all_DLT_coefs(:,cam_ind),cut_wing2_points)...
                ,strel('disk',4))&wing2_bound;
    ...
    if size(indis,1)>=c % if edge alignment is known
        % reconstruct
        x1=indis(c,1);x2=indis(c,2);
        for cam_ind=1:3
            if all_poss_matches(x1,cam_ind)~=cam_ind
                front_tmp=allCams.cams_array(cam_ind).curr_im.wing_r_front_edge;
                back_tmp=allCams.cams_array(cam_ind).curr_im.wing_r_back_edge;
                allCams.cams_array(cam_ind).curr_im.wing_r_front_edge=back_tmp;
                allCams.cams_array(cam_ind).curr_im.wing_r_back_edge=front_tmp;
            end   
            if all_poss_matches(x2,cam_ind)~=cam_ind
                front_tmp=allCams.cams_array(cam_ind).curr_im.wing_l_front_edge;
                back_tmp=allCams.cams_array(cam_ind).curr_im.wing_l_back_edge;
                allCams.cams_array(cam_ind).curr_im.wing_l_front_edge=back_tmp;
                allCams.cams_array(cam_ind).curr_im.wing_l_back_edge=front_tmp;
        ...
        mosquito_all_frames(c).wing_r_front_edge=hull_reconstruction_from_ims(allCams,cam_inds_rec)*allCams.Rotation_Matrix';
        allCams.load_ims_for_reconstruction('wing_r_back_edge'); % updates images an seed for reconstruction
        mosquito_all_frames(c).wing_r_back_edge=hull_reconstruction_from_ims(allCams,cam_inds_rec)*allCams.Rotation_Matrix';
        allCams.load_ims_for_reconstruction('wing_l_front_edge'); % updates images an seed for reconstruction
        mosquito_all_frames(c).wing_l_front_edge=hull_reconstruction_from_ims(allCams,cam_inds_rec)*allCams.Rotation_Matrix';
        allCams.load_ims_for_reconstruction('wing_l_back_edge'); % updates images an seed for reconstruction
        mosquito_all_frames(c).wing_l_back_edge=hull_reconstruction_from_ims(allCams,cam_inds_rec)*allCams.Rotation_Matrix';   
    else
        while ~isitgood
            choosindfig=figure;
%             if (size(indis,1)>=(c-1))&&(c>1)
%                 pp1_val=indis(c-1,1);
%                 pp2_val=indis(c-1,2);
%             else
                pp1_val=1;
                pp2_val=1;
%             end
            pp1 = uicontrol('style','pop',...
             'unit','pix',...
             'position',[10 10 280 20],...
             'fontsize',12,...
             'fontweight','bold',... 
             'string',{'1-123';'2-126';'3-153';'4-156';'5-456';'6-453';'7-426';'8-423'},...
             'value',pp1_val);
            pp2 = uicontrol('style','pop',...
             'unit','pix',...
             'position',[500 10 280 20],...
             'fontsize',12,...
             'fontweight','bold',... 
             'string',{'1-123';'2-126';'3-153';'4-156';'5-456';'6-453';'7-426';'8-423'},...
             'value',pp2_val);
            ims_rem_orig={full(allCams.cams_array(1).curr_im.Image),full(allCams.cams_array(2).curr_im.Image),full(allCams.cams_array(3).curr_im.Image)};
            wing1_edges_fat={allCams.cams_array(1).curr_im.wing_r_front_edge,allCams.cams_array(1).curr_im.wing_r_back_edge;...
                allCams.cams_array(2).curr_im.wing_r_front_edge,allCams.cams_array(2).curr_im.wing_r_back_edge;...
                allCams.cams_array(3).curr_im.wing_r_front_edge,allCams.cams_array(3).curr_im.wing_r_back_edge};
            wing2_edges_fat={allCams.cams_array(1).curr_im.wing_l_front_edge,allCams.cams_array(1).curr_im.wing_l_back_edge;...
                allCams.cams_array(2).curr_im.wing_l_front_edge,allCams.cams_array(2).curr_im.wing_l_back_edge;...
                allCams.cams_array(3).curr_im.wing_l_front_edge,allCams.cams_array(3).curr_im.wing_l_back_edge};
            for cam_ind=1:3
                full_im_with_edges=2^16*wing1_edges_fat{all_poss_matches(pp1_val,cam_ind)}+...
                ims_rem_orig{cam_ind}-2^16*wing2_edges_fat{all_poss_matches(pp2_val,cam_ind)};
            ...
            pp1.Callback={@popup1_callback,ax_ci,wing1_edges_fat,wing2_edges_fat,ims_rem_orig,pp2};
            pp2.Callback={@popup2_callback,ax_ci,wing1_edges_fat,wing2_edges_fat,ims_rem_orig,pp1};

            x1 = input('wing1 inds?');
            if x1~=0 % indices are not already correct
                x2 = input('wing2 inds?');
                if x1~=1
                    for cam_ind=1:3
                        if all_poss_matches(x1,cam_ind)~=cam_ind
                ...
                [~,indx]=ismember([switched(1,1)*3+1,switched(2,1)*3+2,switched(3,1)*3+3],...
                    all_poss_matches,'rows');
                x1=indx;
                [~,indx]=ismember([switched(1,2)*3+1,switched(2,2)*3+2,switched(3,2)*3+3],...
                    all_poss_matches,'rows');
                x2=indx;
            end
            delete(choosindfig);
            ...
            threededgefig=figure;
            hold on
            pcshow([mosquito_all_frames(c).wing_r_front_edge;...
                mosquito_all_frames(c).wing_r_back_edge],'r')
            pcshow([mosquito_all_frames(c).wing_l_front_edge;...
                mosquito_all_frames(c).wing_l_back_edge],'r')
            isitgood = input('is it good?');
            delete(threededgefig)
        end
        indis(c,:)=[x1,x2];
    end
    for cam_ind=1:3
        allCams.cams_array(cam_ind).prev_im=allCams.cams_array(cam_ind).curr_im;
    end
    %% rotate to lab frame
